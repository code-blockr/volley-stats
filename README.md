...
- Every derived number has an ⓘ next to it that opens a plain-English explanation of how it's worked out, followed by the actual weights: what each event is worth in the top of the fraction, and what the whole thing gets divided by. Events worth nothing are still listed as `0`, because "does a 2-Pass count toward good pass %?" is exactly the question the table should answer. It's a tap, not a hover, because `title` attributes do nothing on a phone and the phone is where this gets used.
- The tooltip is `position: fixed` and placed by hand in viewport coordinates. Accordions, cards and table wrappers all clip with `overflow: hidden`, which used to cut the bottom off any bubble opened near the end of a section; fixed escapes every clipping context. It flips above the ⓘ when there isn't room below, clamps to both screen edges, and follows the button on scroll rather than closing - dismissing on scroll sounds tidier until the address bar sliding away on a phone counts as a scroll and shuts the thing the moment you open it.
- All four disciplines live in accordions, attack included, so they read as one set rather than one special case plus three others. Each header keeps its headline number visible while closed, so you can see how you're blocking without opening anything. Whether each is open is remembered per device. On the dashboard they hold all-time numbers, a breakdown doughnut and the trend chart; on a session page, that session's numbers and its doughnut. The four summary cards stay at the top as the at-a-glance line.
- Each discipline gets a breakdown doughnut - what your touches were actually made of. Part-to-whole, never more than five segments, counts and shares in the legend rather than plastered on the slices. The segment colours are a deliberate green→grey→amber→red ramp because these categories are *ordered* (a kill is better than a continue is better than an error), not merely different, so the colour has to carry that direction. The app's own `--success`/`--success-muted` tokens were the obvious first choice and they failed a colourblind-separation check outright - ΔE 7.6 for normal vision, which almost nobody can tell apart as neighbouring slices - so the ramp was re-stepped and re-checked against the dark surface until every adjacent pair cleared both the normal-vision and CVD floors.
